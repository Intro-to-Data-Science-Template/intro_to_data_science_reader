...
.projected_edges = read.csv(here::here("content/class_worksheets/35_networks/data/projected.csv"), header = TRUE, stringsAsFactors = FALSE)

# make plotting df
.projected_df = toy_attributes[!is.na(toy_attributes$color), ]

# plot!
visNetwork(nodes = .projected_df, edges = .projected_edges) %>% visInteraction(zoomView = FALSE)
```
...
```{r echo=FALSE}
visNetwork(toy_attributes, toy_edgelist, main = "High(er) Density") %>% visInteraction(zoomView = FALSE)
```

```{r echo=FALSE}
visNetwork(toy_attributes, toy_edgelist[c(1:5,7,10,13,15),1:2], main = "Low(er) Density") %>% visInteraction(zoomView = FALSE)
```
...
**Geodesic Distance** is "the length of the shortest path via the edges or binary connections between nodes"[^1]. In other words, if we treat the network as a map we can move along, with the nodes being stopping places and the edges being paths, the geodesic is the shortest possible path we can use to walk between two nodes. 

[^1]: Kadushin, C. (2012). Understanding Social Networks: Theories, Concepts, and Findings. Oxford University Press.
...
```{r workflow_data, echo=TRUE, warning=FALSE}
# data load
toy_edgelist = read.csv("https://raw.githubusercontent.com/Intro-to-Data-Science-Template/intro_to_data_science_reader/main/content/class_worksheets/35_networks/data/edgelist.csv", header = TRUE, stringsAsFactors = FALSE)
toy_attributes = read.csv("https://raw.githubusercontent.com/Intro-to-Data-Science-Template/intro_to_data_science_reader/main/content/class_worksheets/35_networks/data/attributes.csv", header = TRUE, stringsAsFactors = FALSE)
